...
var draw_self_collision_boundary: bool = false
...
func _draw():
	...
	if draw_self_collision_boundary:
		for position in points:
			draw_circle(position.position, self_collision_distance / 2, Color8(128, 128, 128, 50))


func create(rows: int, columns: int, width: float, height: float):
	points = []
	positions = []
	velocities = []
	fixed = []
		...
		for j in range(i + 1, n_points):
			...
			if overlap_len > 0:
				...
				if dir.dot(velocities[j] - velocities[i]) < 0:
					velocities[i] -= (dir * dir.dot(velocities[i]) - dir * dir.dot(velocities[j]) * 0.5) * 1
					velocities[j] -= (dir * dir.dot(velocities[j]) - dir * dir.dot(ivel) * 0.5) * 1
